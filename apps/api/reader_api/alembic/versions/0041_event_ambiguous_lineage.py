"""Record conservative ambiguous Event successors and overlap lineage."""
...
revision: str = "0041_event_ambiguous_lineage"
down_revision: str | None = "0040_merge_transaction_index"
...
def upgrade() -> None:
    ...
    op.execute(
        "CREATE UNIQUE INDEX uq_cluster_event_projection_ambiguous_event "
        "ON cluster_event_projections (clustering_run_id, event_id) "
        "WHERE reconciliation_kind = 'ambiguous'"
    )
    op.execute(
        "CREATE INDEX ix_cluster_event_projection_ambiguous_transaction "
        "ON cluster_event_projections (created_transaction_id, event_id) "
        "WHERE reconciliation_kind = 'ambiguous'"
    )
    op.execute(
        "CREATE INDEX ix_cluster_event_projection_ambiguous_event "
        "ON cluster_event_projections (event_id) "
        "WHERE reconciliation_kind = 'ambiguous'"
    )
    op.execute(
        "CREATE INDEX ix_event_lineage_ambiguous_transaction "
        "ON event_lineages "
        "(created_transaction_id, source_event_id, target_event_id) "
        "WHERE relation_type = 'ambiguous_from'"
    )

    op.execute(
        r"""
        CREATE FUNCTION reader_ambiguous_projection_matches_snapshot(
        ...
        CREATE FUNCTION reader_ambiguous_projection_has_newborn_target(
            candidate cluster_event_projections
        )
        RETURNS boolean
        LANGUAGE sql
        VOLATILE
        AS $reader$
            SELECT reader_ambiguous_projection_matches_snapshot(candidate)
               AND candidate.created_transaction_id = pg_current_xact_id()
               AND EXISTS (
                    SELECT 1
                    FROM events target
                    JOIN event_revisions target_revision
                    ...
                    WHERE target.id = candidate.event_id
                      ...
                      AND target.current_revision_id = target_revision.id
        ...
        CREATE FUNCTION reader_ambiguous_lineage_matches_projection(
    ...
    op.execute(
        ...
        BEGIN
            IF TG_OP = 'INSERT' THEN
                ...
                END IF;
                IF NEW.reconciliation_kind = 'ambiguous'
                   AND NOT reader_ambiguous_projection_has_newborn_target(NEW) THEN
        ...
        BEGIN
            ...
            ELSIF NEW.relation_type = 'merged_from' THEN
                ...
                END IF;
            ELSIF NEW.relation_type = 'ambiguous_from' THEN
                IF NOT reader_ambiguous_lineage_matches_projection(NEW) THEN
                    RAISE EXCEPTION 'event_lineage_projection_mismatch: ambiguous Lineage 只能记录 run 中可证明的 before/after overlap 与冻结指纹';
                END IF;
            ELSE
    ...
    op.execute(
        r"""
        CREATE FUNCTION reader_ambiguous_projection_has_complete_terminal_graph(
            candidate cluster_event_projections
        )
        RETURNS boolean
        LANGUAGE plpgsql
        ...
        DECLARE
            parent record;
            unresolved_parent_count integer := 0;
        ...
        BEGIN
            ...
            FOR parent IN
                ...
                WHERE frozen.run_id = candidate.clustering_run_id
                  AND NOT EXISTS (
                      ...
                      WHERE resolved.clustering_run_id = frozen.run_id
                  ...
                  AND NOT EXISTS (
                      SELECT 1 FROM event_lineages resolved_lineage
                      WHERE resolved_lineage.clustering_run_id = frozen.run_id
            ...
            LOOP
                unresolved_parent_count := unresolved_parent_count + 1;
                IF NOT EXISTS (
                ...
                IF EXISTS (
                    ...
                    WHERE before_member.run_id = candidate.clustering_run_id
                      AND before_member.snapshot_phase = 'before'
                      AND before_member.cluster_anchor = parent.cluster_anchor
            ...
            IF unresolved_parent_count < 1 THEN
                RETURN false;
            END IF;
            IF (
                SELECT count(*)
        ...
        CREATE FUNCTION reader_require_complete_ambiguous_projection(
            candidate cluster_event_projections
        )
        RETURNS void
        LANGUAGE plpgsql
        ...
        BEGIN
            IF candidate.reconciliation_kind = 'ambiguous'
               AND NOT reader_ambiguous_projection_has_complete_terminal_graph(
                           candidate
                       ) THEN
        ...
        CREATE FUNCTION reader_validate_current_ambiguous_for_event(
        ...
        BEGIN
            ...
            FOR projection IN
                ...
                WHERE candidate.reconciliation_kind = 'ambiguous'
                  AND candidate.created_transaction_id = pg_current_xact_id()
                  AND (
            ...
            LOOP
                PERFORM reader_require_complete_ambiguous_projection(projection);
            END LOOP;
        END
        $reader$;

        CREATE FUNCTION reader_ambiguous_terminal_projection_guard()
        RETURNS trigger LANGUAGE plpgsql AS $reader$
        BEGIN
            PERFORM reader_require_complete_ambiguous_projection(NEW);
            PERFORM reader_validate_current_ambiguous_for_event(NEW.event_id);
            RETURN NULL;
        END
        $reader$;

        CREATE FUNCTION reader_ambiguous_terminal_event_guard()
        RETURNS trigger LANGUAGE plpgsql AS $reader$
        BEGIN
            PERFORM reader_validate_current_ambiguous_for_event(NEW.id);
            RETURN NULL;
        END
        $reader$;

        CREATE FUNCTION reader_ambiguous_terminal_revision_guard()
        RETURNS trigger LANGUAGE plpgsql AS $reader$
        BEGIN
            PERFORM reader_validate_current_ambiguous_for_event(NEW.event_id);
            RETURN NULL;
        ...
        CREATE FUNCTION reader_ambiguous_terminal_lineage_guard()
        RETURNS trigger LANGUAGE plpgsql AS $reader$
        BEGIN
            PERFORM reader_validate_current_ambiguous_for_event(
                NEW.source_event_id
            );
            PERFORM reader_validate_current_ambiguous_for_event(
                NEW.target_event_id
        ...
        CREATE FUNCTION reader_ambiguous_terminal_user_state_guard()
        RETURNS trigger LANGUAGE plpgsql AS $reader$
        BEGIN
            PERFORM reader_validate_current_ambiguous_for_event(NEW.event_id);
            RETURN NULL;
        ...
        CREATE FUNCTION reader_ambiguous_terminal_baseline_guard()
        RETURNS trigger LANGUAGE plpgsql AS $reader$
        BEGIN
            PERFORM reader_validate_current_ambiguous_for_event(
        ...
        CREATE FUNCTION reader_ambiguous_terminal_interaction_guard()
        RETURNS trigger LANGUAGE plpgsql AS $reader$
        BEGIN
            PERFORM reader_validate_current_ambiguous_for_event(NEW.event_id);
            RETURN NULL;
        ...
        CREATE CONSTRAINT TRIGGER trg_ambiguous_terminal_projection
        AFTER INSERT ON cluster_event_projections
        DEFERRABLE INITIALLY DEFERRED FOR EACH ROW
        EXECUTE FUNCTION reader_ambiguous_terminal_projection_guard();

        CREATE CONSTRAINT TRIGGER trg_ambiguous_terminal_event
        AFTER INSERT OR UPDATE ON events
        DEFERRABLE INITIALLY DEFERRED FOR EACH ROW
        EXECUTE FUNCTION reader_ambiguous_terminal_event_guard();

        CREATE CONSTRAINT TRIGGER trg_ambiguous_terminal_revision
        AFTER INSERT ON event_revisions
        DEFERRABLE INITIALLY DEFERRED FOR EACH ROW
        EXECUTE FUNCTION reader_ambiguous_terminal_revision_guard();

        CREATE CONSTRAINT TRIGGER trg_ambiguous_terminal_lineage
        AFTER INSERT ON event_lineages
        DEFERRABLE INITIALLY DEFERRED FOR EACH ROW
        EXECUTE FUNCTION reader_ambiguous_terminal_lineage_guard();

        CREATE CONSTRAINT TRIGGER trg_ambiguous_terminal_user_state
        AFTER INSERT OR UPDATE ON event_user_states
        DEFERRABLE INITIALLY DEFERRED FOR EACH ROW
        EXECUTE FUNCTION reader_ambiguous_terminal_user_state_guard();

        CREATE CONSTRAINT TRIGGER trg_ambiguous_terminal_baseline
        AFTER INSERT OR UPDATE ON migration_baselines
        DEFERRABLE INITIALLY DEFERRED FOR EACH ROW
        EXECUTE FUNCTION reader_ambiguous_terminal_baseline_guard();

        CREATE CONSTRAINT TRIGGER trg_ambiguous_terminal_interaction
        AFTER INSERT ON interaction_events
        DEFERRABLE INITIALLY DEFERRED FOR EACH ROW
        EXECUTE FUNCTION reader_ambiguous_terminal_interaction_guard();
        """
    )
...
def downgrade() -> None:
    raise RuntimeError(
        "Event ambiguous Lineage 与未决终态约束不支持原地 downgrade；"
        "回滚必须恢复迁移前备份"
    )
