"""Require terminal Clustering Run times to follow the start time."""
...
revision: str = "0022_terminal_time_order"
down_revision: str = "0021_failed_before_seals"
...
def upgrade() -> None:
    op.create_check_constraint(
        "ck_clustering_run_terminal_time_order",
        "clustering_runs",
    ...
    op.execute(
        ...
        BEGIN
            ...
            END IF;
            IF (NEW.completed_at IS NOT NULL AND NEW.completed_at < NEW.started_at)
               OR (NEW.failed_at IS NOT NULL AND NEW.failed_at < NEW.started_at) THEN
                RAISE EXCEPTION 'clustering_run_terminal_time_order: 终态时间不得早于开始时间';
            END IF;
...
def downgrade() -> None:
    raise RuntimeError(
        "Clustering Run terminal time order 不支持原地 downgrade；回滚必须恢复迁移前备份"
    )
