"""Require verifiable before and after snapshot seals for completion."""
...
revision: str = "0020_before_snapshot_seals"
down_revision: str = "0019_snapshot_seals"
...
def upgrade() -> None:
    op.execute(
        "DROP TRIGGER trg_clustering_snapshot_seal "
        "ON clustering_run_snapshot_seals"
    )
    op.execute(
        """
        ...
        SELECT
            ...
            COALESCE(run.completed_at, run.failed_at, CURRENT_TIMESTAMP)
        ...
        GROUP BY run.id, run.completed_at, run.failed_at
        ON CONFLICT (run_id, snapshot_phase) DO NOTHING
        """
    )
    op.execute(
        """
        ...
        BEGIN
            ...
            IF NEW.status = 'completed' THEN
                FOREACH required_phase IN ARRAY ARRAY['before', 'after'] LOOP
                    ...
                    IF NOT FOUND THEN
                        RAISE EXCEPTION 'clustering_run_snapshot_unsealed: completed 必须引用已封印的 % snapshot', required_phase;
                    END IF;
                    SELECT
                        count(*)::integer,
                        encode(
...
def downgrade() -> None:
    raise RuntimeError(
        "Clustering Run before snapshot seal 不支持原地 downgrade；回滚必须恢复迁移前备份"
    )
